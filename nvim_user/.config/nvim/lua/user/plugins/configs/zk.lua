...
function M.grep_notes()
  require("user.plugins.configs.zk.note").grep_notes({ dir = vim.env.ZK_NOTEBOOK_DIR })
end

function M.open_notebook()
  require("user.plugins.configs.zk.note").open_notebook()
end

function M.find_or_create_note(opts)
  require("user.plugins.configs.zk.note").find_or_create_note(opts)
end

function M.find_or_create_note_without_picker(opts)
  require("user.plugins.configs.zk.note").find_or_create_note_without_picker(opts)
end

function M.find_or_create_project_note()
  require("user.plugins.configs.zk.note").find_or_create_project_note()
end

function M.open_notes()
  require("user.plugins.configs.zk.note").open_notes()
end

function M.annotate_task()
  require("user.plugins.configs.zk.note").annotate_task()
end
...
function M.init()
  vim.cmd([[
        function! OpenJournalNote(day, month, year, weekday, direction)
            ...
            return luaeval('require("user.plugins.configs.zk").__open_journal_note(_A[1], _A[2], _A[3], _A[4], _A[5])',
...
function M.show_calendar()
  ...
  require("user.plugins.configs.calendar").show({ dir = journal_dir })
end
