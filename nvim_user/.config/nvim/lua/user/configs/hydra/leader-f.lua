...
function M.setup(Hydra, cmd, _)
  ...
  Hydra({
    ...
    heads = {
      ...
      { "p", "<cmd>lua require('telescope').extensions.project.project{}<CR>", { exit = true, desc = "projects" } },
      {
        "/",
        "<cmd>lua require('telescope.builtin').current_buffer_fuzzy_find()<CR>",
        { exit = true, desc = "search in file" },
      },
      { "?", "<cmd>lua require('telescope.builtin').search_history()<CR>", { desc = "search history" } },
