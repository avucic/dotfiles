return {
  ...
  -- ── text-case ─────────────────────────────────────────────────────────────────
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<Leader>xi.", function()
        textcase_pick("current_word")
      end, { silent = true, desc = "Text case word" })
      vim.keymap.set("v", "<Leader>xi.", function()
        textcase_pick("visual")
      end, { silent = true, desc = "Text case selection" })
    end,
  },
  ...
  -- ── align.nvim ────────────────────────────────────────────────────────────────
  {
    ...
    keys = {
      ...
      },
      {
        ...
        mode = "v",
        function()
          require("align").align_to_char({ preview = true, regex = true })
        end,
        desc = "Regex",
      },
    },
  ...
  -- ── vim-visual-multi ─────────────────────────────────────────────────────────
  {
    ...
    init = function()
      ...
      -- While a VM session is active, let bare j/k add cursors (repeatable,
      -- no need to retype the ,j/,k prefix). Restored on exit.
      vim.api.nvim_create_autocmd("User", {
        pattern = "visual_multi_start",
        callback = function()
          vim.keymap.set("n", "j", "<Plug>(VM-Add-Cursor-Down)", { buffer = true, remap = true })
          vim.keymap.set("n", "k", "<Plug>(VM-Add-Cursor-Up)", { buffer = true, remap = true })
        end,
      })
      vim.api.nvim_create_autocmd("User", {
