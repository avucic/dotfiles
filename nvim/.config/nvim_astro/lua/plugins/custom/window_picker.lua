...
local function window_keys(windows)
  ...
  for _, nr in ipairs(nrs) do
    ...
    if nr ~= current then
      ...
      if mapping[key] then key = key .. (index == #chars and chars[1] or chars[index + 1]) end
...
--     if key ~= start and key:sub(1, 1) == start then return true end
--   end
--
--   return false
-- end
...
function M.load(user_config) config = vim.tbl_extend("force", config, user_config) end
function M.setup(user_config) config = vim.tbl_extend("force", config, user_config) end
